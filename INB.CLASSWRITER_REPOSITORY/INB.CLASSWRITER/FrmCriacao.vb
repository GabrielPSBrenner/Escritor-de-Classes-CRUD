...
#End Region
    ...
    Private Sub CriaArquivosClassesTemplate()
        ...
        For i = 0 To Items.Count - 1
            ...
            If Tabela.TotalCampos > 0 Then
                If TipoClasse = ModelType.ClassesDAO Or TipoClasse = ModelType.ClassesPartial Then
                    oConfig.GerarArquivosModel(Tabela)
                ElseIf TipoClasse = ModelType.Repository Then
                    oConfig.GerarArquivoIRepository(Tabela)
                    oConfig.GerarArquivoRepository(Tabela)
                Else
                    MessageBox.Show("Tipo inexistente.")
                    Exit Sub
                End If
    ...
    Private Sub ExportaCRepository(NomeArquivoCompleto As String, NomeArquivoInterface As String, NomeArquivo As String, Tabela As LERESTRUTURA.TABLE.stTable)
        'Exporta a interface IRepository
        Dim strm As New StreamWriter(NomeArquivoInterface)
        strm.WriteLine("using System;")
        strm.WriteLine("using System.Collections.Generic;")
        strm.WriteLine("using INB.Infra.Data.Interfaces;")


        strm.WriteLine("namespace " & NAMESPACEMODEL & ".Interfaces")
        strm.WriteLine("{")

        strm.WriteLine("   interface IRepository" & NomeArquivo & ":IRepositoryBase<" & Tabela.Nome & "," & lDataContextName & ", T_LOG>")
        strm.WriteLine("   {")
        strm.WriteLine("   }")
        strm.WriteLine("}")
        strm.Close()
        ...
        'Exporta a classe Repository
        strm = New StreamWriter(NomeArquivoCompleto)
        strm.WriteLine("using System;")
        strm.WriteLine("using System.Collections.Generic;")
        strm.WriteLine("using INB.Infra.Data;")
        strm.WriteLine("using INB.Infra.Data.Intefaces;")
        strm.WriteLine("using System.Linq;")
        strm.WriteLine("using System.Text;")
        strm.WriteLine("using System.Threading.Tasks;")
        strm.WriteLine("")
        strm.WriteLine("namespace " & NAMESPACEMODEL & ".Repositories")
        strm.WriteLine("{")
        strm.WriteLine("")
        strm.WriteLine("   public class Repository" & Tabela.Nome & ": RepositoryBase<" & Tabela.Nome & "," & lDataContextName & ", T_LOG>, Interfaces.IRepository" & Tabela.Nome & ", IDisposable")
        strm.WriteLine("   {")
        strm.WriteLine("")
        strm.WriteLine("    private bool _RecebeuDataContext;")
        strm.WriteLine("")
        strm.WriteLine("      public Repository" & Tabela.Nome & "(" & lDataContextName & " DataContext, bool SubmitSaveChanges = true)")
        strm.WriteLine("      : base(DataContext, SubmitSaveChanges)")
        strm.WriteLine("      {")
        strm.WriteLine("         _RecebeuDataContext = true;")
        strm.WriteLine("      }")
        strm.WriteLine("")
        strm.WriteLine("      public Repository" & Tabela.Nome & "(bool SubmitSaveChanges = true)")
        strm.WriteLine("      : base(Helper.DataHelper.GetDataContext(), SubmitSaveChanges)")
        strm.WriteLine("      {")
        strm.WriteLine("         _RecebeuDataContext = false;")
        strm.WriteLine("      }")
        strm.WriteLine("")
        strm.WriteLine("      public Repository" & Tabela.Nome & "(" & lDataContextName & " DataContext, IIdentification UserIdentification, bool SubmitSaveChanges = true)")
        strm.WriteLine("      : base(DataContext, UserIdentification, SubmitSaveChanges)")
        strm.WriteLine("      {")
        strm.WriteLine("         _RecebeuDataContext = true")
        strm.WriteLine("      }")
        strm.WriteLine("")
        strm.WriteLine("      public Repository" & Tabela.Nome & "(IIdentification UserIdentification, bool SubmitSaveChanges = true, bool IgnoreLog = false)")
        strm.WriteLine("      : base(Helper.DataHelper.GetDataContext(), UserIdentification, SubmitSaveChanges, IgnoreLog)")
        strm.WriteLine("      {")
        strm.WriteLine("         _RecebeuDataContext = false;")
        strm.WriteLine("      }")
        strm.WriteLine("")
        strm.WriteLine("      public void Dispose()")
        strm.WriteLine("      {")
        strm.WriteLine("         If (!_RecebeuDataContext) Then")
        strm.WriteLine("              DataContext.Dispose();")
        strm.WriteLine("      }")
        strm.WriteLine("")
        strm.WriteLine("   }")
        strm.WriteLine("")
        ...
        For i = 0 To Items.Count - 1
            ...
            If Tabela.TotalCampos > 0 Then
                If lLinguagemCriacao = LinguagemCriacao.VBNET Then
                    ...
                    ExportaVB_NET(NomeArquivoCompleto, NomeArquivo, Tabela)
                Else
                    If TipoClasse = ModelType.ClassesPartial Then
                        NomeArquivo = Prefixo & NomeArquivo.Substring(NomeArquivo.IndexOf(".") + 1)
                        NomeArquivoCompleto = Diretorio & NomeArquivo.Replace(" ", "_") & ".cs"
                        ExportaC(NomeArquivoCompleto, NomeArquivo, Tabela)
                    ElseIf TipoClasse = ModelType.Repository Then
                        NomeArquivo = Prefixo & "Repository_" & NomeArquivo.Substring(NomeArquivo.IndexOf(".") + 1)
                        NomeArquivoCompleto = Diretorio & NomeArquivo.Replace(" ", "_") & ".cs"
                        NomeInterface = "I" & NomeArquivoCompleto
                        ExportaCRepository(NomeArquivoCompleto, NomeInterface, NomeArquivo, Tabela)
                    End If
                End If
            End If
        Next
